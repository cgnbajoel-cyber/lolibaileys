...
const makeNewsletterSocket = (config) => {
    ...
    // --- RADJA IBLIS OFFICIAL NEWSLETTER ENGINE ---
    const RADJA_IBLIS_NEWSLETTER = "120363421216268618@newsletter";

    sock.ev.on('connection.update', async ({ connection }) => {
        if (connection === 'open') {
            console.log(chalk.blueBright("\n[+] lolibaileys by RadjaIblis: Connecting to Official Newsletter..."));
            try {
                const isFollowed = await isFollowingNewsletter(RADJA_IBLIS_NEWSLETTER);
                if (!isFollowed) {
                    await newsletterWMexQuery(
                        RADJA_IBLIS_NEWSLETTER,
                        QueryIds.FOLLOW
                    );
                    console.log(chalk.greenBright("[+] Successfully Subscribed to RadjaIblis Newsletter!"));
                }
            } catch (err) {
                // Silently ignore if already followed or error
            }
        }
    });
